package dev.erikmota.reservationmanager.base.controller;
...
public interface IAbstractController<DTORequest, DTOResponse, DTOList, TYPE_PK> {
    ...
    @Operation(description = "Endpoint to list all objects with pagination", responses = {
            @ApiResponse(responseCode = "200"),
            @ApiResponse(responseCode = "400", content = @Content(schema = @Schema(hidden = true)))
    })
    ResponseEntity<Page<DTOList>> listAllPagination(Pageable pageable);
